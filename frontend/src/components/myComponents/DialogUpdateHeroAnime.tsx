...
const DialogUpdateHeroAnime = ({ animes = [], heroAnimes = [] }: { animes: Anime[], heroAnimes: IHeroAnime[] }) => {
  const { updateHeroAnime, isLoading } = useHeroAnimeStore();
  ...
  useEffect(() => {
    if (selectedHeroId) {
        const item = heroAnimes.find(h => String(h.id) === selectedHeroId);
        if (item) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateHeroAnime(Number(selectedHeroId), {
      ...
      console.error("Failed to update hero anime:", error);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <ActionCard
          title="Update Hero Anime"
          subtitle="Modify details of an existing hero anime banner"
        />
      </DialogTrigger>
        ...
        <form className="space-y-5 pr-4" onSubmit={handleSubmit}>
          <DialogHeader className="flex flex-row items-center gap-3 space-y-0 pb-2">
            ...
            <div>
                ...
                Select a hero anime to update its details.
                </p>
            </div>
          </DialogHeader>
          ...
          <Card className="bg-white/5 border border-white/5 rounded-2xl shadow-inner">
            <CardContent className="p-4">
                <div className="space-y-2">
                    <label htmlFor="select_hero" className="text-sm font-medium text-slate-300">
                    Select Hero Anime <span className="text-amber-400">*</span>
                    </label>
                    <Select
                        value={selectedHeroId}
                        onValueChange={(value) => setSelectedHeroId(value)}
                    >
                    <SelectTrigger className="bg-black/40 border-white/10 text-white rounded-xl transition-all duration-300 focus-visible:ring-2 focus-visible:ring-amber-500/50 focus-visible:ring-offset-0 focus-visible:border-amber-500 hover:border-amber-500/50">
                        <SelectValue placeholder="Choose a hero anime to update" />
                    </SelectTrigger>
          ...
          <div className="flex items-center justify-end gap-3 pt-1 pb-1">
            ...
            <Button
              type="submit"
              disabled={isLoading || !selectedHeroId}
            ...
            >
              {isLoading ? "Updating..." : "Update Hero Anime"}
            </Button>
          </div>
        </form>
         </ScrollArea>
...
export default DialogUpdateHeroAnime;
